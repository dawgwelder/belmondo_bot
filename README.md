...
- **Игровое меню** — `/game`: выбор «Шпионской операции», «Создай алиби», «Продай это Бельмондо» или «Рулетки»
- **Цитаты** — `/quote`
...
| `/game`             | Меню игр: три групповые LLM-игры и рулетка                   |
...
Команда `/game` показывает меню с кнопками «Шпионская операция», «Создай алиби», «Продай это Бельмондо» и «Рулетка». Первые три пункта создают LLM-лобби в текущем чате. Автор лобби запускает игру после набора минимум двух участников; максимум — восемь. Все игровые ответы принимаются только reply-сообщениями на текущий prompt раунда, поэтому обычные триггеры и AI-реплаи не перехватывают ход игрока.
