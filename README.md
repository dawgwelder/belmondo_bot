...
активности, поэтому события не появляются серией. Первые три разных пользователя
получают по агенту; каждый claim, награда и history фиксируются одной SQLite
transaction. Повторный клик не занимает дополнительное место. Registry также включает Dead Drop, Intercept, Chase, cooperative
