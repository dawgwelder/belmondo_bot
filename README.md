...
подбирает трёхзначный код по серверным подсказкам за шесть попыток. Для каждой
операции SQLite хранит одну попытку на пользователя, а награду атомарно получает
первый победитель. Без `SPY_GAME_HTML5_URL` сохраняются прежние Telegram-кнопки.
