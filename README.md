...
Событие «Перехват» опционально запускается как Telegram HTML5 Game с short name
`spies`. Игрок настраивает пять частот; сервер пересчитывает результат, хранит
одну попытку на пользователя в SQLite и атомарно выдаёт предмет первой успешной
попытке. Без `SPY_GAME_HTML5_URL` сохраняется прежний текстовый Перехват.
