...
export SPY_GAME_NARRATOR_GENERATION_COOLDOWN_SECONDS=300
export SPY_GAME_NARRATOR_REFRESH_SECONDS=21600
# Генерация дел «Крота» включена по умолчанию, независимо от рассказчика.
export SPY_GAME_LLM_MOLE_ENABLED=true
export SPY_GAME_LLM_MOLE_TIMEOUT_SECONDS=15
...
Рассказчик постепенно пополняет кэш до восьми вариантов на тип, тон и сюжетный
контекст, затем обновляет его по одному тексту после шестичасовой паузы.
Генерация запускается не чаще раза в пять минут на всю базу; ошибки и дубликаты
тоже расходуют эту попытку. Интервалы задаются переменными
`SPY_GAME_NARRATOR_GENERATION_COOLDOWN_SECONDS` и
`SPY_GAME_NARRATOR_REFRESH_SECONDS`. Внутри одной генерации structured JSON
transport допускает до двух обращений к модели. Три последние завязки в каждом
чате исключаются из выбора; история и текст конкретного события сохраняются
между перезапусками. Без LLM доступны восемь локальных вариантов на тип события
с тем же исключением повторов. Миграция 017 применяется автоматически при старте.
...
Оба директора исключают третье одинаковое событие подряд, если есть альтернатива,
и гарантируют набор после трёх других опубликованных событий. Локальный директор
снижает вес недавних событий, учитывает активность для групповых операций и
выбирает тон отдельным случайным броском. LLM получает те же веса как подсказку;
ограничения последовательности проверяются сервером. Сюжетное открытие «Крота»
сохраняется, повторное дело не вытесняет все остальные события.
