...
Слот-машина доступна постоянно в Mini App: вкладка «Слоты» и кнопка в досье.
Она не создаёт событий и работает во время других операций. Ставки — 1/3/5
осведомителей; правила и выплаты описаны в руководстве игрока. Результат выбирает
сервер, списание и выплата фиксируются одной SQLite-транзакцией. Повтор запроса
с тем же `operation_id` возвращает сохранённый результат; изменение ставки или
чата для этого ID отклоняется. История и пауза между вращениями общие для игрока
во всех чатах. Миграция `018_slot_spins.sql` применяется при старте; дополнительных
флагов, процессов и событий для доступа не требуется. Как и другие операции
с ресурсами в Mini App, вращение требует Telegram-авторизации и контекста
включённой группы из `/spy`. Прямое открытие профиля остаётся режимом просмотра.
...
данных Telegram. Групповой контекст backend подбирает сам по сохранённому
членству пользователя в активированных чатах (`chat_members`, обновляется по
сообщениям, inline-кнопкам и `/spy`), поэтому полный кабинет открывается из
меню и профиля бота, по чужой кнопке `/spy` и после restart. Кнопка группового
`/spy` содержит лишь короткоживущую подписанную подсказку чата в `startapp`;
при нескольких чатах Mini App показывает переключатель и передаёт выбор opaque
handle без числовых ID. Публичный
...
и, в v3, один бонусный Tier 3 либо Tier 4 при ставке минимум пяти агентов
выбранного тира или выше. Без доступного бонуса победа всё равно даёт ×2;
старые v1/v2 сохраняют прежние награды. После контрольной точки доступна
